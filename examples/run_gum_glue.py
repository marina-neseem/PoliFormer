...
from transformers.modeling_gum_bert import GumBertForSequenceClassification as BertForSequenceClassification
...
def train(args, train_dataset, model, tokenizer, do_pretrain_controller_train=False):
    """ Train the model """
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if do_pretrain_controller_train:
                inputs['do_pretrain_controller_train'] = True
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    tb_writer.add_scalar('loss_model', loss1, global_step)
                    tb_writer.add_scalar('loss_policy', loss2, global_step)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        with open(output_eval_file, "w") as writer:
            ...
            for key in sorted(result.keys()):
                ...
                writer.write("%s = %s\n" % (key, str(result[key])))
            logger.info("  average executed layers = %f \n", 100*total_executed_layers/(len(eval_dataloader)*model.bert.config.num_hidden_layers))
            writer.write("average executed layers = %f \n" % (100*total_executed_layers/(len(eval_dataloader)*model.bert.config.num_hidden_layers)))
...
def main():
    ...
    parser.add_argument("--target_compute", default=0.5, type=float,
                        help="Target compute percentage.")
    parser.add_argument("--efficiency_weight", default=0.1, type=float,
    ...
    if args.do_pretrain_controller_train:
        ...
        for n, p in model.bert.encoder.controller.named_parameters():
            p.requires_grad = True
            print("unfreezing", n)

        train_dataset = load_and_cache_examples(args, args.task_name, tokenizer, \
                            evaluate=False)
        global_step, tr_loss = train(args, train_dataset, model, tokenizer, do_pretrain_controller_train=True)
        logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)
        args.do_train = True
